...
struct Doctor: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        check("settings.json 주입") {
            ...
            return (false, "없음. clfctl settings install --port 51710")
        }

        check("Keychain 접근") {
            let probe = KeychainCredentialStore(service: "me.clf.doctor-probe")
        ...
        check("등록된 조직") {
            ...
            if doc.accounts.isEmpty {
                return (false, "없음. clfctl accounts add <id>")
            }
            return (auto > 0,
        ...
        for id in doc.priority {
            check("자격증명 \(id)") {
                store.hasCredential(for: id)
                    ? (true, "있음")
                    : (false, "Keychain 에 없음. clfctl accounts add \(id) 로 다시 넣는다")
            }
        }
