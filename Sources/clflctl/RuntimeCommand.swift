    ...
    struct Simulate: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            ...
            한도에 실제로 걸릴 때까지 기다리지 않고 선택기 동작을 확인한다.
              ...
              clflctl runtime simulate team1 --rate-limit 300
              clflctl runtime simulate team1 --session-limit 1800
              clflctl runtime simulate team1 --invalid
              clflctl runtime simulate team1 --headroom 0.03
            """)
        @OptionGroup var paths: Paths
        ...
        func run() async throws {
            ...
            await file.schedule(runtime)
            try await file.flush()
            print("  \(id) 상태를 바꿨다. clflctl select 로 확인한다")
        }
    }
}
