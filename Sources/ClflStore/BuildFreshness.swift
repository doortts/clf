...
///
/// `~/.local/bin/clflctl` 이 `.build/release/clflctl` 를 직접 가리키는 심볼릭
...
public enum BuildFreshness {
    ...
    public static var packageRoot: URL? {
        // <root>/Sources/ClflStore/BuildFreshness.swift
    ...
    public static func warning(executable: URL, sourceRoot: URL) -> String? {
        ...
        return """
              주의: clflctl 이 낡았다. 소스가 \(age(seconds: changed.timeIntervalSince(built))) 더 새롭다
