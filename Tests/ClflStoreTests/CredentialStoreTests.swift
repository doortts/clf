...
final class KeychainCredentialStoreTests: XCTestCase {
    let service = "me.clfl.test.\(UUID().uuidString)"
    lazy var store = KeychainCredentialStore(service: service)

    override func setUpWithError() throws {
        try super.setUpWithError()
        guard ProcessInfo.processInfo.environment["CLFL_KEYCHAIN_TESTS"] == "1" else {
            throw XCTSkip("CLFL_KEYCHAIN_TESTS=1 일 때만 돈다. 잠금 해제 프롬프트를 띄운다")
        }
    }
