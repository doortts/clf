...
final class BundleLocationTests: XCTestCase {
    ...
    func test_buildDirectoryIsNotAHome() {
        XCTAssertFalse(LoginItemState.isStableLocation("/Users/x/repos/clfl/.build/clfl.app"))
    }

    func test_applicationsFoldersAreFine() {
        XCTAssertTrue(LoginItemState.isStableLocation("/Applications/clfl.app"))
        XCTAssertTrue(LoginItemState.isStableLocation("/Users/x/Applications/clfl.app"))
    }

    /// 개발 중에는 여기서 돈다. 승인을 받아도 다음 make-app.sh 에 사라진다.
    func test_derivedDataIsNotAHome() {
        XCTAssertFalse(LoginItemState.isStableLocation(
            "/Users/x/Library/Developer/Xcode/DerivedData/clfl-abc/Build/Products/Debug/clfl.app"))
    }
}
...
final class LoginItemMappingTests: XCTestCase {
    let home = "/Users/x/Applications/clfl.app"
    ...
    func test_locationLosesToNothing() {
        for raw in 0...3 {
            XCTAssertEqual(
                LoginItemState.from(statusRawValue: raw, path: "/Users/x/repos/clfl/.build/clfl.app"),
                .unavailable, "raw=\(raw)")
        }
    }
