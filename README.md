...
무채색 구간은 "노랑이 곧 온다" 는 예고 자리다. 예전에는 50% 부터였는데
그러면 절반쯤 쓴 창이 벌써 물러나 보였다. 주간 Fable 창은 여유 구간에서만
초록 대신 파랑이라 세 줄이 색으로 갈린다.

---

## 설치

받아서 쓰려면 [릴리즈 페이지](https://github.com/doortts/clf/releases)에서
DMG 를 받아 `clf.app` 을 `~/Applications` 에 끌어다 놓는다. 애플 공증을
받은 빌드라 경고 없이 열린다. macOS 13 이상.
...
Swift 6 툴체인이 필요하다. Xcode 프로젝트는 없다.
...
- **리셋 표기**: 막대의 숫자 옆에 남은 시간 / 리셋 시각
- 로그인할 때 실행
- 한도가 바닥나면 알림
- 멀티 인스턴스 정보 삭제: 계정별 창이 쌓아 둔 `~/.claude-alt-*` 를 지운다
...
데스크톱 알림은 한 계정에 세 번이다. **빨강에 들어설 때**(잔여 5% 이하),
**다 썼을 때**, 그리고 **리셋으로 다시 쓸 수 있게 됐을 때**.
...
릴리즈는 `scripts/release.sh` 가 태그부터 GitHub 릴리즈까지 한 판에 한다.
Developer ID 서명과 애플 공증(앱과 DMG 각각)이 들어가므로 인증서와
notarytool 키체인 프로필이 필요하다. 스크립트 머리 주석에 준비물이 있다.

```
CLF_SIGN_IDENTITY="Developer ID Application: ..." ./scripts/release.sh v0.1.0
```
