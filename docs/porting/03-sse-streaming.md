...
([02](02-response-classification.md) §2 참고). status만 보고 통과시키면 이걸 놓친다.
...
**peek은 파싱이 아니다.** 이 단계는 프레임 *경계*만 찾는다. 필드 파싱은 §4의 파서가 한다.
...
**호출자의 관심사**이고, 에러 프레임을 버퍼로 내리는 경로(§6)에서만 상한을 건다.
...
재생해야 하기 때문**이다([02](02-response-classification.md) §6, 6단계). 라이브 스트림은
재생할 수 없다.
...
func relayStreaming(
    ...
    do {
        ...
        while let chunk = try await iterator.next() {
            ...
            //     건넌 뒤에는 계정을 바꿀 수 없다.
            //   - 첫 프레임 에러는 §6의 버퍼 강등이 이미 처리했다. 스트림 중간
        ...
        //      에러 body 작성을 금지한다
        try? await client.abort()   // ⚠️ end()가 아니다 — 아래 참고
        return nil                  // 호출자는 usage 기록을 건너뛴다
    }
}
```

### ⚠️ 중간 실패 시 `end()`가 아니라 강제 종료
