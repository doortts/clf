...
| 4 | `proxy/swap.ts`의 결정 루프 | ~200 | [02](02-response-classification.md) 4절 | 1,474줄 중 루프 뼈대만. 나머지는 알림/dedupe |
| 5 | `sse-parser.ts` + `stream-peek.ts` | 415 | [03](03-sse-streaming.md) | 알고리즘만 참고. 구현은 NIO에 위임 |
| 6 | `stream-writer.ts` | 330 | [03](03-sse-streaming.md) 5절 | once-write 불변식과 중단 처리 |
...
앱에서는 2.가 해소되고 3.이 역전된다. 영구 타임라인이 있으면 "주기적으로 다시 경고해서
...
|---|---|---|---|---|---|
| **Swift + SwiftUI** | `MenuBarExtra` - 네이티브 | swift-nio | X | ~10MB | **채택** |
| Tauri 2 (Rust) | 트레이 지원, popover는 덜 네이티브 | hyper/tokio | O | ~15MB | Windows 필수 시 대안 |
| Flutter (Dart) | 플러그인 의존 | dart:io | O | ~45MB | 기각 |
| Electron (Node) | 무난 | claulay 코드 그대로 | O | ~200MB | 상주 앱엔 과중 |
