...
실제로 T52 에서 T40 으로 옮긴 뒤 하루가 지나도록 쿠키에 T52 가 남아 있었고,
막대와 팝오버가 그동안 T52 를 가리켰다.

`config.json` 에 계정마다 이런 평문 키가 하나씩 있다.

```
dxt:allowlistLastUpdated:746e81ae-...  2026-08-31T03:03:26.824Z
dxt:allowlistLastUpdated:2a063dae-...  2026-08-30T15:38:59.896Z
```

앱이 그 계정을 열 때 확장 허용목록을 받아 적는 시각이다. 쓰기는 앱 안에서
일어나므로 서버 응답을 안 기다린다. 별도 창 디렉토리 셋을 대조해 보니 가장
최근 값이 그 창이 쓰는 계정과 셋 다 맞았다.

그래서 `DesktopReader` 는 **쿠키를 먼저 보고, 쿠키가 가리키는 계정보다 더
최근에 연 계정이 있으면 그쪽으로 옮긴다.** 쿠키의 계정에 시각이 없으면
견줄 것이 없으므로 쿠키를 그대로 믿는다.
