...
| **쿠키 쓰기 (앱 종료 후) + 재시작** | **부팅 조직은 바뀐다. 유지되지 않는다** | 3절, 6절 |
...
23:46:28  [LocalAgentModeSessionManager] Initialization succeeded
          accountId=914e4f12-..., orgId=746e81ae-...
```

복원된 대화 세션이 자기 조직(`746e81ae` = `NAVER_TEAM_40`)을 갖고 있고, 그 세션이
활성화되는 순간 앱 전체가 그쪽으로 재초기화된다. 디스크 쿠키도 함께 되돌아갔다.

**조직은 앱 전역 상태가 아니라 세션에 딸린 값이다.** 쿠키는 "열려 있는 세션이
없을 때의 기본값"일 뿐이다.
...
| 세션 없는 상태에서 부팅 조직 정하기 | **된다.** 쿠키 쓰기 + 재시작 |
| 열려 있는 세션의 조직 바꾸기 | **안 된다.** 세션이 자기 조직으로 되돌린다 |
| 재시작 없이 전환 | 좌표 클릭뿐. CDP 는 막혔다 |

읽기는 깨끗하게 되고 쓰기는 세션에 진다. 자동 전환을 하려면 **드롭다운을 좌표로
클릭하는 것 외에 방법이 없다.** 그 취약함은 [08 검증](08-verification.md) 7-4절에
적은 그대로다.
