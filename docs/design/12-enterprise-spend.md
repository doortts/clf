...
팀 조직은 5시간, 주간 전체, 주간 Fable 세 창으로 묶인다. Enterprise 는 아니다.

---

## 1. 실측

Enterprise 조직의 Usage API 응답이다. 앱에서 그 조직을 한 번 열어 토큰이
캐시된 뒤 딱 한 번 불렀다.
...
조직마다 **시간 창 셋** 또는 **월 예산 하나** 중 하나를 그린다.
...
| 갱신 주기 지문 | 예산 사용액도 넣는다. 시간 창이 없는 조직은 그것만 움직인다 |
| 낡은 값 물려주기 | 예산도 함께 물려준다 |
| 메뉴바 숫자 | `5h`/`1w` 두 줄 대신 `예산` 한 줄 |

시간 창이 없는 조직에 `5h` 를 그리면 그 자체가 거짓말이다.
