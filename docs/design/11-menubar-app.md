...
사각형 다섯, 사각형마다 세로선 넷. 스무 칸이므로 **선 하나가 5%** 다.
5% 단위로 **올림**한다.
...
실제 크기에서는 선 넷이 붙어 사각형 하나로 읽힌다. **그게 의도다.** 흘깃 볼
때는 덩어리 수가 답이고, 확대하면 값이 어디서 끊겼는지 눈금으로 보인다.

**선끼리는 붙이고 사각형끼리만 뗀다.** 선 1pt 넷이 붙어 4pt 사각형이 되고
사각형 사이만 1pt 띄운다. 전체 폭 24pt, 줄 사이 1.5pt.

선 사이에 틈을 두면 실제 크기에서 사각형이 안 보이고 잔금만 보인다. 눈금은
값이 어디서 끊겼는지 **채운 폭으로** 드러나면 된다. 선을 갈라 그릴 이유가 없다.

처음에는 사각형 5pt 에 선 0.5pt, 간격 1pt 로 잡았다. 안쪽 여백이 0.75pt =
1.5px 라 선이 반 픽셀에 걸쳐 뭉개졌고, 정작 사각형 사이만 벌어져 보였다.
정렬을 고쳤더니 이번에는 선이 낱낱이 보였다. 둘 다 아니고 답은 붙이는
쪽이었다.

줄 사이를 두는 것은 붙여 두면 세 줄이 한 덩어리로 보여 어느 창이 어느 줄인지
못 읽기 때문이다. 1.5pt = 3px 면 갈라 보이면서 덩어리로도 읽힌다. 2pt 는
넓었다.

숫자 왼쪽에 `5h` 와 `1w` 를 붙인다. 셋째 줄은 자리가 없어 게이지만 남는다.

숫자 자리는 `100%` 네 글자에 맞춘다. 좁게 잡았더니 `%` 가 다음 줄로 떨어졌다.
