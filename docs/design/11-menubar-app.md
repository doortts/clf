...
잔여를 담은 알약이 왼쪽, 테두리를 두른 게이지가 오른쪽이다. 게이지는 바깥에
등급색 테두리만 두르고 안쪽을 그만큼 채운다. **사이에 틈을 둔다.** 없으면
가득 찼을 때 테두리가 사라져 100% 와 그냥 채워진 막대가 구별이 안 된다.

알약 글자색은 등급마다 다르다. 초록과 노랑은 밝아서 검은 글자, 빨강은 흰
글자다. 정상 구간은 알약을 안 칠하고 기본 라벨색을 쓴다.

1% 라도 남았으면 채움이 보이게 최소 폭을 준다. 0 으로 그리면 소진과 같아진다.
