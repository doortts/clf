...
메뉴바 블록은 칸을 떨어뜨려 놓지만 팝오버 게이지는 다르다. 막대 하나 안에서
**점의 밀도**로 채움을 표현한다. 채운 쪽은 촘촘하고 빈 쪽은 성기다.
...
격자 2pt = 4px, 채운 점 1.5pt = 3px, 빈 점 0.5pt = 1px. 전부 2배 화면에서
정수 픽셀이라 번지지 않는다. **점을 칸 한가운데 두면 0.25pt 같은 값이 나와
다시 흐려진다.** 채운 점은 칸 왼쪽 위에 붙이고 빈 점만 안쪽 픽셀에 놓는다.

높이는 네 줄 8pt 다. 시안은 6pt 라고 적었는데 디더로 그리면 세 줄은 선처럼
얇아 보인다. 질감이 보여야 게이지로 읽힌다.
