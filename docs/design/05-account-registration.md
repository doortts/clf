...
> 이 문서는 **터미널 트랙**이다. 메뉴바 앱은 계정을 따로 등록받지 않고
> 데스크톱 앱이 이미 캐시해 둔 토큰을 읽는다. [10 문서](10-desktop-usage.md) 2절.
>
...
그대로 뿜는다. 그래서 이런 것들이 자격증명에 섞여 들어왔다.

| 섞인 것 | 어디서 왔나 |
|---|---|
| `U+001B` (ESC) | setup-token 의 색상, 커서 이동 코드 |
| `U+E0B2`, `U+F080` | 터미널 프롬프트의 Powerline, Nerd Font 장식 |
| 줄바꿈 | 터미널 접힘이나 프롬프트 다음 줄 |

오염된 값을 저장하면 두 가지가 따라온다. 업스트림이 401 을 내고 사용자는 토큰이
틀렸다고 생각한다. 그리고 `security find-generic-password -w` 가 출력 불가
바이트를 보고 원문 대신 16진수를 내므로 읽기까지 깨진다.

### 도구가 뽑아낸다

사용자가 grep 파이프라인을 만들 일이 아니다. `accounts add` 가 stdin 을 받아
ANSI 시퀀스를 걷어내고 `sk-ant-` 로 시작하는 값이나 `claudeAiOauth` 블록을 찾는다.

**값을 고치지는 않는다. 찾아낼 뿐이다.** 후보가 정확히 하나일 때만 통과시킨다.

| 상황 | 결과 |
|---|---|
| 깨끗한 토큰 하나 | 그대로 저장 |
| ANSI, 장식, 여러 번 다시 그린 출력 | 뽑아내고 그 사실을 알린다 |
| 서로 다른 토큰 둘 이상 | 거부. 우리가 고를 수 없다 |
| 토큰 없음 | 거부 |
| 40글자 미만 | 거부. 접히며 잘렸을 수 있다 |
| 필드 빠진 oauth 블록 | 거부. 캡처가 덜 됐다 |

읽는 쪽도 `security` 의 16진수 출력을 알아본다. 우리 형식은 `L:` 이나 `O:` 로,
Claude CLI 슬롯은 `{` 로 시작하므로 소문자 16진수 집합과 겹칠 수 없다.
판정이 결정적이다.
