...
1. **5-2 (7일 창)** 를 먼저 고친다. 우리 시안이 이미 그 케이스를 그리고 있는데
   로직이 못 본다. 가장 명백한 구멍이다
2. **5-1 (hysteresis)**, **5-3 (샘플 만료)**, **5-4 (쿨다운)** 를 02 문서에 반영한다
3. **6절의 포지셔닝**을 README 에 반영한다. clfl 이 무엇을 위한 도구인지 좁혀 말한다
4. **5-5 (auth login 토큰)** 는 1차 범위 밖으로 두되 05 문서에 선택지로 기록한다
5. CCSwitcher 를 실제로 설치해 써본다. 문서만으로는 체감이 안 되는 부분이 있고,
   써보고 나서도 clfl 이 필요하면 그때 근거가 분명해진다
