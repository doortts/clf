...
### 왜 커밋 해시가 아니라 수정 시각인가

해시를 박아 넣으려면 빌드 플러그인이 필요하고, 커밋 안 한 변경은 못 잡는다.
개발 중에 낡는 건 대부분 커밋 전이다.

`Sources` 와 `Package.swift` 만 본다. `.build` 는 항상 최신이라 세면 늘 낡았다고
나오고, `docs` 와 `Tests` 는 바이너리 동작과 무관하다. 디렉토리 자체의 시각도
세지 않는다. 파일이 추가되면 그 파일 시각으로 잡히기 때문이다.

### 지키는 선

| | |
|---|---|
| 막지 않는다 | 경고만 한다. 낡은 바이너리로도 하려던 일은 대개 된다 |
| stderr 로 보낸다 | `--json` 출력에 섞이면 파이프가 깨진다 |
| 소스가 없으면 조용하다 | 배포된 바이너리 옆에는 소스 트리가 없다 |
| 실패해도 죽지 않는다 | 검사 하나 때문에 도구가 안 뜨면 안 된다 |

소스 루트는 `#filePath` 로 컴파일 시점에 정해진다. 그 자리에 `Package.swift` 가
없으면 남의 디렉토리이므로 뒤지지 않는다.

한 가지 흠이 있다. 내용은 그대로 두고 `touch` 만 하면 SPM 이 재링크를 건너뛰어
경고가 안 사라진다. 다음 실제 수정 때 풀린다. 다시 빌드해서 손해 볼 것은 없다.
