...
| | 하는 일 | |
|---|---|---|
| CLI `--fork-session` | 메시지 uuid 를 전부 재생성, 부모 사슬 재연결, `forkedFrom` 기록, sidechain/progress 제거 | `--resume` 이라 한 턴을 쓸 수 있고 CLI 자격증명을 탄다 |
| 데스크톱 앱 Fork | `copyFile` 로 줄을 그대로 복사 | 배포되어 돌고 있다 |
| 우리 | 앱과 같게 | 한 턴도 안 쓰고 즉시 끝난다 |

CLI 쪽이 더 깨끗하지만 그만한 값을 치른다. **앱이 감수하고 쓰는 방식이면
우리도 감수할 수 있다.** 파일 이름과 안의 `sessionId` 가 어긋나는 것도 앱이
이미 그렇게 하고 있다.

### 짝 없는 도구 호출만 떼어낸다

복사본이 안 열리는 경우가 하나 있다. assistant 가 `tool_use` 를 냈는데
`tool_result` 가 없는 상태로 끝난 파일이다. API 가 거부한다.

처음에는 `stop_reason: end_turn` 까지 자르려 했다. **틀린 생각이었다.**
...
에이전트 세션은 한 턴에 도구를 수십 번 부른다. `end_turn` 까지 자르면 진행
중이던 작업 전체를 버린다. 그리고 한도는 대개 그런 긴 턴 중간에 걸린다.

실제로 깨진 것만 뗀다.

| 끝 모양 | |
|---|---|
| `assistant` end_turn | 유효 |
| `user` tool_result | **유효.** 한도에 걸린 모습이 이것이다 |
| `user` 텍스트 | 유효. 새 계정에서 그 답부터 받는다 |
| `assistant` 짝 없는 tool_use | 이것만 뗀다 |

한도에 걸리는 순서를 따지면 도구 실행은 로컬이라 한도와 무관하다. 다음
요청에서 걸리므로 트랜스크립트는 `tool_result` 로 끝나 있다. **주 용도에서는
한 줄도 안 잘린다.**

그래서 이 판정은 UI 에 안 드러낸다. 실제로 뗐을 때만 말한다.
...
```json
{"customTitle":"[넘김] 기본 산술 계산","sessionId":"...","type":"custom-title"}
```

앱은 `customTitle` 을 `aiTitle` 보다 먼저 본다. 그래서 이 한 줄이 목록에 뜨는
이름을 정한다. 두 번 넘겨도 표시가 겹치지 않게 이미 붙어 있으면 그대로 둔다.

`sessionId` 는 파일 안의 값을 그대로 쓴다. 줄을 안 고치는 방식이므로 제목
줄만 새 id 를 쓰면 혼자 어긋난다.
