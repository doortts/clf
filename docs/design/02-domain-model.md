...
```
T1 44%   T2 12%          <- 실제로는 코드와 숫자가 붙는다
   91%      31%
```

- 코드 뒤에 숫자를 **두 줄**로 쌓는다. 위가 5시간 사용률, 아래가 7일 사용률
- 코드와 숫자 사이에 공백을 두지 않는다. 두 줄로 쌓인 형태가 이미 구분이 된다
- 두 계정 사이에 분리선을 긋지 않는다. 여백과 투명도만으로 가른다
- 직전 계정은 투명도를 낮춰 확실히 뒤로 물린다

**두 줄인 이유**는 전환이 5시간과 7일 중 어느 쪽이 찼느냐에 따라 일어나기 때문이다.
하나만 보여주면 왜 넘어왔는지가 절반 가려진다. 5시간이 47% 로 여유가 있는데 7일이
100% 라서 넘어온 경우, 5시간만 보면 이유를 알 수 없다.

한도에 걸린 줄만 색으로 표시한다. 계정 전체를 물들이지 않는다.

| 상태 | 표시 |
|---|---|
| 대기, 사용 중 | 5시간과 7일 사용률 두 줄 |
| 쿨다운 | 두 줄 유지. 찬 쪽이 100% 로 보이므로 그 자체가 이유가 된다 |
