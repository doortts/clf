...
> 이 문서는 **터미널 트랙**이다. 계정 선택과 쿨다운은 프록시가 쓰는 것이고,
> 지금 제품인 메뉴바 앱은 여기 있는 판단을 쓰지 않는다.
>
...
- 작은 크기에서 솔리드 막대보다 눈금이 읽힌다. 4pt 짜리 실선은 색만 보이지만
  도트는 남은 칸 수가 보인다
- **채운 점을 빈 점보다 굵게 그린다.** 빈 점 반지름 1.05pt, 채운 점 1.35pt.
  색보다 무게 차이가 먼저 읽히므로 색맹 사용자에게도 경계가 보인다
- **NSProgressIndicator 의 기본 모양은 아니다.** 의도적인 이탈이다

#### 도트는 시스템 색을 그대로 쓸 수 없다

1pt 짜리 점은 안티에일리어싱에 색이 씻긴다. 면으로 칠할 때 멀쩡하던 값이 점으로는
흐려 보인다. 그래서 막대 전용 색을 따로 둔다.

| | 라이트 | 대비 | 다크 | 대비 | 메뉴바(항상 어두움) | 대비 |
|---|---|---|---|---|---|---|
| 여유 | `#17993f` | 3.7 | `#3ce16b` | 9.7 | `#3ce16b` | 8.3 |
| 경고 | `#bf7f00` | 3.4 | `#ffe500` | 13.0 | `#ffe500` | 11.2 |
| 소진 | `#e02017` | 4.8 | `#ff4a3d` | 5.0 | `#ff4a3d` | 4.3 |

대비는 각 바탕 기준 명도 대비비다. 그래픽 요소 최소선인 **3:1 을 전부 넘긴다.**

라이트 모드 경고색이 노랑이 아니라 앰버인 것은 물리적으로 불가피하다. systemYellow
(`#ffcc00`)는 흰 바탕에서 대비가 2.1 밖에 안 나와 점으로 그리면 거의 안 보인다.
노랑으로 보이는 것보다 보이는 것이 먼저다.

빈 점은 대비 1.4 ~ 1.5 로 일부러 낮게 둔다. 배경이지 정보가 아니다.
