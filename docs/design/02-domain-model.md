...
public enum MenuBarDisplayMode: String, Codable, Sendable {
    /// 기본값. 사용 중인 조직의 세 창을 막대와 숫자로. 약 215pt
    case bars
    /// 사용 중 조직과 직전 조직의 숫자만. 약 90pt
    case numbers
    /// 사용 중 조직의 코드 하나. 약 45pt
    case codeOnly
}
```
...
| 모드 | 보이는 것 | 직전 조직 |
|---|---|---|
| `bars` (기본) | 5시간, 주간, 모델별 주간을 막대 24x4pt 와 숫자 10pt 로 | 안 보인다. 폭이 없다 |
| `numbers` | 두 줄 숫자 (5시간, 주간) | 보인다 |
| `codeOnly` | 코드만 | 안 보인다 |

셋 다 **사용 중인 조직을 먼저** 보여준다. 다른 점은 폭과 정보량의 맞바꿈뿐이다.

`bars` 에서는 **막대가 색을 지고 숫자는 기본색으로 둔다.** 둘 다 물들이면 노이즈가 된다.
잔여가 0% 여도 막대 채움에 `min-width: 2pt` 를 줘서 빈 자리와 소진을 구별한다.

`bars` 에서 직전 조직을 빼는 것은 폭 때문이다. 전환은 코드가 바뀌는 것으로 드러나고,
떠나온 조직의 상태는 팝오버에서 본다.
